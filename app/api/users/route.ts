...
export async function POST(req: Request) {
  try {
    ...
    if (type === "signup") {
      ...
      const newUser = await UserModel.create({ email, user, password, id: count + 1, avatar: null, contact: null });

      return NextResponse.json({ message: "Signup successful", user: newUser });

    } else if (type === "login") {
      const existingUser = await UserModel.findOne({ email }) || await UserModel.findOne({ user: email });
